...
const ResourceManage = () => {
  ...
  const { initialState, refresh } = useModel('@@initialState'); //获取修改的全局变量
  const { canButton } = useAccess(); // 按钮权限控制
  // 把按钮挂到对应菜单下面
  const attachButtonsToTree = (tree: any[], buttons: any[]) => {
    const map = new Map();
    ...
    // 2️⃣ 把按钮挂到对应父节点
    buttons.forEach((btn) => {
      const parent = map.get(btn.parentId);
      if (parent) {
        parent.children = parent.children || [];
        parent.children.push(btn); // 👈 挂进去
      }
    });
          ...
          >
            ...
            <ProFormTreeSelect
              ...
              request={async () => {
                ...
                return [{ id: 0, resourceName: '根节点', children: treeData }]; //默认返回根节点以及资源
              }}
              fieldProps={{
            ...
            {({ resourceType }) => {
              ...
              return (
                <ProFormText
                  ...
                  label="路径/URL"
                  placeholder={
                    isButton ? '按钮类型无需填写路径' : '菜单必填，如 /sys/user'
                  }
                  disabled={isButton} // 如果是按钮，直接禁用输入框
                  // 如果是按钮，自动将值设为 null 或空，防止误填残留数据
                  fieldProps={{
